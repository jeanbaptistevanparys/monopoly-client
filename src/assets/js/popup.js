...
function showRolledDicePopup(numbers, funct) {
	const $template = document
		.querySelector('#roll-dice')
		.content.firstElementChild.cloneNode(true);
	$template.querySelector('.roll-dice form p').innerText =
		numbers[0] + numbers[1];
	$template.querySelector(
		'.roll-dice section img'
	).src = `assets/media/dice/${numbers[0]}.png`;
	$template.querySelector(
		'.roll-dice section img + img'
	).src = `assets/media/dice/${numbers[1]}.png`;
	$template
		.querySelector('.roll-dice form input')
		.addEventListener('click', funct);
	_$popupContainer.insertAdjacentElement('beforeend', $template);
}
...
function showPlayerInfoPopup(playername, properties) {
	...
	$template
		.querySelectorAll('.playerpopup .icons div')
		.forEach((e) => e.addEventListener('click', closePopup));
	const $prop = $template
		.querySelector('.playerpopup .properties section')
		.cloneNode(true);
	$template.querySelector('.playerpopup .properties section').remove();
	properties.forEach((property) => {
		const $propcopy = $prop.cloneNode(true);
		...
		$template
			.querySelector('.playerpopup .properties')
			.insertAdjacentElement('beforeend', $propcopy);
	});
	_$popupContainer.insertAdjacentElement('beforeend', $template);
}
...
function showTitledeedPopup(streetname, properties) {
	...
	$template
		.querySelectorAll('.titledeed .icons div')
		.forEach((e) => e.addEventListener('click', closePopup));
	const $prop = $template.querySelectorAll('.titledeed-content .values p');
	$template
		.querySelectorAll('.titledeed-content .values p')
		.forEach((e) => e.remove());
	const keylist = [
		'rentWithOneHouse',
		'rentWithTwoHouses',
	...
	Object.keys(properties).forEach((key) => {
		if (keylist.includes(key)) {
			const p1 = $prop[0].cloneNode(true);
			p1.innerText = `-${key}`;
			const p2 = $prop[1].cloneNode(true);
			p2.innerHTML = `<span class="striketrough">M</span> ${prop[key]}`;
			$template
				.querySelector('.titledeed-content .values')
				.insertAdjacentElement('beforeend', p1);
			$template
				.querySelector('.titledeed-content .values')
				.insertAdjacentElement('beforeend', p2);
		}
	});
	_$popupContainer.insertAdjacentElement('beforeend', $template);
}
...
function showTradePopup(playername, gamestate) {
	...
	const $template = document
		.querySelector('#trade')
		.content.firstElementChild.cloneNode(true);
	$template.querySelector('.trade-content .trade-names h2').innerText =
		playername;
	$template
		.querySelectorAll('.trade .icons div')
		.forEach((e) => e.addEventListener('click', closePopup));
	...
	gamestate['players'].forEach((player) => {
		if (playername !== player.name) {
			...
			$template
				...
				.insertAdjacentElement('beforeend', $opt);
		}
	});
	loadOptions(
		$template.querySelector('.tradables ul + ul'),
		gamestate,
		$template.querySelector(selector).value
	);
